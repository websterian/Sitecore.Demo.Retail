...
function initActiveLoyaltyCards(sectionId) {
    ClearGlobalMessages();
    AJAXPost("api/storefront/loyalty/activeLoyaltyCards", null, function (data, success, sender) {
...
function initLoyaltyCards(sectionId) {
    ...
    AJAXPost("/api/storefront/loyalty/getLoyaltyCards", null, function (data, success, sender) {
        if (success && data.Success) {
            loyaltyCardListViewModel = new LoyaltyCardsListViewModel(data);
            ko.applyBindings(loyaltyCardListViewModel, document.getElementById(sectionId));
            $("#" + sectionId).show();
...
function joinLoyaltyProgram() {
    ...
    AJAXPost("api/storefront/loyalty/activateAccount", null, function (data, success, sender) {
        if (success && data.Success) {
